                ...
                switch (type) {
                    ...
                    case YEAR:
                        ...
                        if (complicationType == ComplicationData.TYPE_LONG_TEXT) {
                            if (_selection.contains(MONTH_NUMBER) && _selection.contains(DAY)) {
                                line += DATE_SEPARATORS[separator] + String.format(Locale.US, "%02d", calendar.get(Calendar.MONTH) + 1);
                                line += DATE_SEPARATORS[separator] + String.format(Locale.US, "%02d", calendar.get(Calendar.DAY_OF_MONTH));
                                _selection.remove(DAY);
                                _selection.remove(MONTH_NUMBER);
                            }
                            if (_selection.contains(MONTH_TEXT) && _selection.contains(DAY)) {
                                line += DATE_SEPARATORS[separator] + MonthToString(calendar.get(Calendar.MONTH)).substring(0, 3);
                                line += DATE_SEPARATORS[separator] + String.format(Locale.US, "%02d", calendar.get(Calendar.DAY_OF_MONTH));
                                _selection.remove(DAY);
                                _selection.remove(MONTH_TEXT);
                            }
                        }
                        break;
                    case MONTH_NUMBER:
                        line = String.format(Locale.US, "%02d", calendar.get(Calendar.MONTH) + 1);
                        if (_selection.contains(DAY) && _selection.size() > 2) {
                            line += DATE_SEPARATORS[separator] + String.format(Locale.US, "%02d", calendar.get(Calendar.DAY_OF_MONTH));
                            _selection.remove(DAY);
                        }
                        break;
